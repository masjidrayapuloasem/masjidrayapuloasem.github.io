...
export function PrayerSchedule() {
  ...
  const fetchPrayerTimes = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://api.aladhan.com/v1/timings/${day}-${month}-${year}?latitude=${LOCATION.latitude}&longitude=${LOCATION.longitude}&method=20`);
      if (!response.ok) {
  ...
  return <section id="jadwal" className="py-20 lg:py-32 bg-muted">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16 stagger-children">
          ...
          {/* Refresh Button */}
          <button onClick={fetchPrayerTimes} disabled={loading} className="mt-4 inline-flex items-center gap-2 text-sm text-primary hover:text-primary/80 transition-colors disabled:opacity-50">
        ...
        Array.from({
          length: 5
        }).map((_, index) => <div key={index} className={`relative bg-card rounded-2xl p-6 shadow-card border border-border text-center ${index === 0 ? "col-span-2 md:col-span-1" : ""}`}>
              ...
              </div>) : prayerTimes.map((prayer, index) => <div key={prayer.name} className={`relative bg-card rounded-2xl p-6 shadow-card border border-border text-center group hover:shadow-elevated transition-all duration-300 ${index === 0 ? "col-span-2 md:col-span-1" : ""}`}>
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-primary flex items-center justify-center shadow-soft">
                  <Clock className="w-5 h-5 text-primary-foreground" />
                </div>
        ...
        {/* Error Message */}
        {error && <p className="text-center text-destructive text-sm mt-4">
            {error} - Menampilkan jadwal default
          </p>}

        {/* Last Updated */}
        {lastUpdated && !error && <p className="text-center text-muted-foreground text-xs mt-6">
